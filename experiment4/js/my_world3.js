...
function drawPlanets(i, j, hash) {
  ...
  if (shouldDrawPlanet) {
    ...
    // You can adjust these coordinates to place the planets where you want
    let planetX = 0;
    let planetY = 0;
...
function drawRedCircles(i, j, hash) {
  ...
  if (shouldDrawRedCircle) {
    ...
    let gradientColors = [color(255, 150, 0), color(255, 0, 0)]; // Orange and red gradient colors
    let gradientRadius = circleSize / 2; // Radius of the gradient
    let gradientCenterX = circleX + cos(millis() / 1000) * gradientRadius; // X-coordinate of the center of the gradient
    let gradientCenterY = circleY + sin(millis() / 1000) * gradientRadius; // Y-coordinate of the center of the gradient
    radialGradient(circleX, circleY, gradientColors, gradientRadius, gradientCenterX, gradientCenterY);
